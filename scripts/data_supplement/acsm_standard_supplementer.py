...
class ACSMStandardSupplementer:
    ...
    def __init__(self, neo4j_driver: AsyncDriver, data_dir: str = "data/training_knowledge"):
        ...
        self.standards_dir = self.data_dir / "acsm_standards"
    ...
    async def supplement(self) -> SupplementResult:
        ...
        logger.info("开始ACSM标准补充")
        
        try:
            # 1. 检查目录是否存在
            if not self.standards_dir.exists():
                logger.error(f"ACSM标准目录不存在: {self.standards_dir}")
                return SupplementResult(
                    total_nodes=0,
                    created_nodes=0,
                    errors=[{"stage": "check_dir", "error": "ACSM标准目录不存在"}],
                    execution_time=time.time() - start_time
                )
            
            # 2. 读取所有JSON文件
            json_files = list(self.standards_dir.glob("*.json"))
            logger.info(f"找到 {len(json_files)} 个ACSM标准文件")
            
            if not json_files:
                logger.warning("未找到ACSM标准文件")
                return SupplementResult(
                    total_nodes=0,
                    created_nodes=0,
                    errors=[],
            ...
            async with self.driver.session() as session:
                ...
                for json_file in json_files:
                    try:
                        ...
                        if exists:
                            logger.debug(f"跳过已存在的ACSM标准: {json_file.name}")
                            skipped_count += 1
                            continue
                        
                        # 创建ACSMStandard节点
                        created = await self._create_acsm_standard(
                            session,
                            standard_id,
                            json_file.name,
                            standard_data
            ...
            logger.error(f"ACSM标准补充失败: {e}", exc_info=True)
            execution_time = time.time() - start_time
            return SupplementResult(
                total_nodes=0,
                created_nodes=0,
                errors=[{"stage": "acsm_standard_supplement", "error": str(e)}],
                execution_time=execution_time
            )
    ...
    async def _create_acsm_standard(
        self,
        session,
        standard_id: str,
        file_name: str,
    ...
    def _translate_title(self, data_type: str) -> str:
        ...
        """
        translations = {
            "fitt_principle": "FITT原则",
            "heart_rate_guidelines": "心率指南",
            "resistance_training": "抗阻训练",
            "cardiovascular_training": "心血管训练",
            "flexibility_training": "柔韧性训练"
        }
