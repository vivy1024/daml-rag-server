...
class ExerciseSupplementer:
    ...
    def _load_kinetic_chain_rules(self) -> Dict[str, str]:
        """加载运动链分类规则"""
        return {
    ...
    async def supplement(self, use_ollama: bool = False, batch_size: int = 100) -> SupplementResult:
        ...
        try:
            async with self.driver.session() as session:
                # 0. 数据质量检查
                logger.info("\n步骤0：数据质量检查")
                quality_report = await self._check_data_quality(session)
                
                # 1. 补充运动链类型
                logger.info("\n步骤1：补充运动链类型")
                await self._supplement_kinetic_chain_type(session)
                
                # 2. 补充技术检查点（关键动作）
                logger.info("\n步骤2：补充技术检查点（关键动作）")
                await self._supplement_technique_checkpoints_manual(session)
                
                # 2.5. 使用Ollama生成技术检查点（可选）
                if use_ollama:
                    logger.info("\n步骤2.5：使用Ollama生成技术检查点")
                    await self._supplement_technique_checkpoints_with_llm(session, batch_size)
                
                # 3. 补充关节活动度要求
                logger.info("\n步骤3：补充关节活动度要求")
                await self._supplement_rom_requirements(session)
                
                # 统计结果
                total_nodes = await self._count_exercise_nodes(session)
                updated_nodes = await self._count_updated_nodes(session)
                
                logger.info("\n" + "=" * 60)
                logger.info("Exercise节点数据补充完成")
    ...
    async def _check_data_quality(self, session) -> Dict[str, Any]:
        """
        检查数据质量问题
        ...
        if duplicate_count > 0:
            ...
            """
            mark_result = await session.run(mark_query)
            mark_record = await mark_result.single()
            logger.info(f"✅ 已标记 {mark_record['marked_count']} 个动作")
        
        return quality_report
    
    async def _supplement_kinetic_chain_type(self, session):
        """
        补充运动链类型
        ...
        for keyword in closed_chain_keywords:
            ...
            if record["updated_count"] > 0:
                logger.info(f"   闭链动作 '{keyword}': {record['updated_count']} 个")
        ...
        for keyword in open_chain_keywords:
            ...
            record = await result.single()
            if record["updated_count"] > 0:
                logger.info(f"   开链动作 '{keyword}': {record['updated_count']} 个")
        ...
        result = await session.run(query)
        record = await result.single()
        logger.info(f"   混合动作: {record['updated_count']} 个")
        
        logger.info("✅ 运动链类型补充完成")
    
    async def _supplement_technique_checkpoints_manual(self, session):
        """
        为关键动作手动添加技术检查点
        ...
        for exercise_name, checkpoints in self.technique_templates.items():
            ...
            if record["updated_count"] > 0:
                logger.info(f"   ✅ {exercise_name}: {len(checkpoints)} 个检查点")
        
        logger.info("✅ 关键动作技术检查点补充完成")
    ...
    async def _supplement_technique_checkpoints_with_llm(self, session, batch_size: int = 100):
        ...
        exercises = await result.data()
        
        if not exercises:
            logger.info("   ℹ️ 没有需要生成技术检查点的动作")
            return
        ...
        for i, exercise in enumerate(exercises, 1):
            try:
                ...
                failed_count += 1
                logger.warning(f"   ⚠️ 为动作 {exercise['name']} 生成检查点失败: {e}")
...
"""
    ...
    async def _supplement_rom_requirements(self, session):
        """
        补充关节活动度要求
        ...
        for exercise_name, rom in self.rom_requirements.items():
            ...
            for joint, angle in rom.items():
                if not (0 <= angle <= 180):
                    logger.warning(f"   ⚠️ {exercise_name} 的 {joint} 角度 {angle} 超出范围")
    ...
    async def _count_exercise_nodes(self, session) -> int:
        """统计Exercise节点总数"""
    ...
    async def _count_updated_nodes(self, session) -> int:
        """统计已更新的节点数"""
        query = """
        MATCH (e:Exercise)
