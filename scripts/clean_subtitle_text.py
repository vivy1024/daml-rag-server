...
ORAL_CONNECTOR_PATTERNS = [
    ...
    # "OK" 作为口语过渡（保留技术语境中的OK）
    (re.compile(r'(?<=[，。！？\s])OK\s*(?=[，。\s]|$)', re.IGNORECASE), '', 'OK(过渡)'),
]

# --- P3: 冗余表达 ---
REDUNDANT_PATTERNS = [
    # "我们说" / "我们看" / "大家看" 作为口语引导 → 删除
    (re.compile(r'(?<=[，。])\s*我们说\s*'), '', '我们说'),
    (re.compile(r'(?<=[，。])\s*我们看\s*'), '', '我们看'),
    (re.compile(r'(?<=[，。])\s*大家看\s*'), '', '大家看'),
    # "就是说" → "即"（非句首时）
    (re.compile(r'(?<=[\u4e00-\u9fff])就是说(?=[\u4e00-\u9fff])'), '即', '就是说'),
    # "也就是说" → "即"
...
TRAILING_PARTICLE_PATTERNS = [
    # 句尾 "啊" 在逗号/句号前（但保留"吗""呢"等有语法功能的）
    (re.compile(r'啊(?=[，。])'), '', '啊(句尾)'),
    # 句尾 "哈" 在逗号/句号前
    (re.compile(r'哈(?=[，。])'), '', '哈(句尾)'),
    # 句尾 "呢" 在句号前（问句中的"呢"保留，只去句号前的）
    # 注意：这个比较激进，暂时不启用
]
