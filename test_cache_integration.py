...
def test_cache_integration():
    ...
    # 测试4：验证API路由的feature flag逻辑
    print("\n【测试4】验证API路由的feature flag逻辑")
    print("-" * 80)
    
    try:
        from api.routes.user import _use_new_cache as api_use_new_cache
        use_new_api = api_use_new_cache()
        print(f"API路由 _use_new_cache()返回值: {use_new_api}")
        print(f"预期: False（使用旧预加载器）")
        print(f"结果: {'✅ 通过' if not use_new_api else '❌ 失败'}")
    except Exception as e:
        print(f"❌ API路由feature flag测试失败: {e}")
